        ...
        foreach (Event::$eventParamsNames as $key => $value) {
            ...
            if (in_array($value, $columns))
                ...
                */
                ...
                if ($value=="eventType") // 1. Typ // Erste Zeile schmaler. GW 140923; 
                    $output .= "<th class='thinner'>".$key."</th>"; 
                else if ($value=="title_long") // 2. Schulungsthema // Zweite Spalte breiter, alle anderen schmaler. GW 141118; 
                    $output .= "<th class='title_long'>".$key."</th>";
                else if ($value=="duration_min")  // 3. Dauer
                    $output .= "<th class='duration_min'>".$key."</th>"; 
                else if ($value=="max_participants")  
        ...
        foreach ($events as $eventRow) {
            ...
            */
            ...
            if(isset($certificate_date) && time()<$rep_date) {
                ...
                foreach ($eventRow as $key => $value) {
                    ...
                    if (in_array($key, $columns)) {
                        if ($key == 'title_long')
                            ...
                            else if (($key == 'max_participants') && ($show_elearning_link==1))
                                $value = "";
                            else if (($key == 'max_participants') && ($show_elearning_link!=1))
                ...
                if($onetime===false) {$output .= "<tr><td colspan='7'><center><strong>Due Courses</strong></td></tr><tr>";$onetime=true;}
...
<tr><td></td><th><a href = '?page=$editTopicPage&order=topicID'>TopicID</a></th>";
    ...
    public static function eventActionOutput($eventActionsPage, $participantsStatus, $eventID) {
        ...
        /* GW 141006 */
        $eventType = eventDatabaseManager::getEventType($eventID);
        $contentLink = eventDatabaseManager::getContentLink($eventID);
        if ($eventType=="elearning")
            return "<a href = '".$contentLink."' class='sign_in_link'>START</a>";
        /* / GW 141006 */
...
<table>";
        ...
        if ($locationID != null) {
            ...
            $output .= "<tr><td><b>" . LOCATION_NAME . "</b></td><td>" . View::textFormOutput('locationName', $name) . "</td></tr>";
        }
        else
