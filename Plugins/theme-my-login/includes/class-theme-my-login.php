			...
			switch ( $this->request_action ) {
				...
				case 'rp' :
					...
					if ( isset( $_COOKIE[ $rp_cookie ] ) && 0 < strpos( $_COOKIE[ $rp_cookie ], ':' ) ) {
						...
						if ( isset( $_POST['upass1'] ) && ! hash_equals( $rp_key, $_POST['rp_key'] ) ) {
							$user = false;
						}
					} else {
						$user = false;
					...
					if ( isset( $_POST['upass1'] ) && $_POST['upass1'] != $_POST['upass2'] )
					...
					if ( ( ! $this->errors->get_error_code() ) && isset( $_POST['upass1'] ) && ! empty( $_POST['upass1'] ) ) {
						self::reset_password( $user, $_POST['upass1'] );
						setcookie( $rp_cookie, ' ', time() - YEAR_IN_SECONDS, $rp_path, COOKIE_DOMAIN, is_ssl(), true );
