...
class EventDatabaseManager {
     ...
     * GW 150310 Do nothing is $userID is not set  
     *
     * @global wpdb $wpdb
     * @param int $userID
     * @return Array
     ...
     * GW 150310 Do nothing if UserID or topicID are not set
     * 
     * @global wpdb $wpdb
     * @param int $userID
     * @param int $topicID
    ...
    public static function getTopicPerCourse() {
        ...
        foreach ($res as $course => $coursesUsers) {
            ...
            foreach ($res1 as $users1 => $user1) {
                ...
                $res[$fez]['users'][$fez1]['topic_expiry_date'] .= $user1["topic_expiry_date"]; // Bis wann ein Topic bezhlt ist
     ...
     public static function isTopicForUserActive($userID, $topicID) {
        ...
        $query = "  SELECT * 
                    ...
                    ON (( cert.cert_user_id = uht.userID ) AND (cert.cert_course_id = cht.courseID))
                    LEFT JOIN datr_Course_has_Topic cht 
...
// Neu am 03.07.2015
        ...
        if ($date!="") 
            ...
            $str .= "<span style=\"background: orange; color: white;\">".$date."</span>";
            $str .= " + ";
            $str .= $res[0]['repetition_frequency_days'];
            $str .= " = ";
            $rep_date = strtotime($res[0]['cert_generated']." +".$res[0]['repetition_frequency_days']." days")."=";
            $rep_date = date("d.m.Y",$rep_date);
            $str .= "<span style=\"background: #FFCC00; color: black;\">".$rep_date."</span>";

            //$str .= strtotime($res[0]['cert_generated']);
//$str .= "-".strtotime('today');
//$str .= "=".(strtotime($date = $res[0]['cert_generated']) - strtotime('today'));
            
             if ($res[0]['repetition_frequency_days']==0) 
                {
                ...
                {
                if (strtotime($res[0]['cert_generated']." +".$res[0]['repetition_frequency_days']." days") - strtotime('today') < 0) 
                    {
                    // vergangen
                    $str .= " <span style=\"background: red; color: white;\">[V: zu bearbeiten]</span>";
                    }  
                if (strtotime($res[0]['cert_generated']." +".$res[0]['repetition_frequency_days']." days") - strtotime('today') >= 0) 
                    {
                    // heute oder zukunft
                    $str .= " <span style=\"background: green; color: white;\">[Z: done]</span>";
            ...
            {
            $str .= " <span style=\"background: red; color: white;\">[K: zu bearbeiten]</span>";
            }   
    ...
    /**
     * get CourseID by Topic
     * Returns $courseID on success, nothing otherwise;
     * @global wpdb $wpdb
     * @param int $topicID
     * @return int
     */
    public static function getCourseByTopic($topicID) {
        global $wpdb;
       
        $query = "SELECT courseID 
                  FROM datr_Course_has_Topic 
    ...
    /**
     * get certificate_date by courseID and userID
     * Returns $certificate_date on success, nothing otherwise;
     * @global wpdb $wpdb
     * @param int $courseID;
     * @param int $userID;
     * @return date
     */
    public static function getCertificateDate($userID,$courseID) {
        global $wpdb;
        
        $query = "SELECT cert_generated 
                  FROM wp13_wpcw_certificates 
                  WHERE cert_user_id = $userID
                  AND cert_course_id = $courseID";
        
        $res = $wpdb->get_results($query, ARRAY_A);
        
        return $res[0]["cert_generated"];
    }
